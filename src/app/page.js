...
const CartDrawer = ({ isOpen, onClose, cartItems, onRemoveItem, clearCart }) => {
  ...
  useEffect(() => { if (!isOpen && step === 'success') { setStep('cart'); setOrderId(null); setAppliedCoupon(null); setCouponCode(''); } }, [isOpen]);
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (data.success) { setAppliedCoupon({ code: data.data.code, discount: data.data.discount_percent }); setCouponCode(''); } 
      else { setCouponError('Invalid Coupon'); setAppliedCoupon(null); }
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {step === 'checkout' && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-10 duration-300">
                ...
                <div className="bg-blue-50 p-4 rounded-lg mt-4 space-y-2"><div className="flex justify-between text-sm text-blue-700"><span>Subtotal</span><span>৳ {subtotal}</span></div><div className="flex justify-between text-sm text-blue-700"><span>Shipping</span><span>৳ {shipping}</span></div>{appliedCoupon && <div className="flex justify-between text-sm text-green-700 font-bold"><span>Discount ({appliedCoupon.code})</span><span>- ৳ {discountAmount}</span></div>}<div className="flex justify-between text-base text-blue-900 font-bold border-t border-blue-200 pt-2 mt-2"><span>Total (COD)</span><span>৳ {total}</span></div></div>
                {!appliedCoupon ? ( <div className="flex gap-2"><input type="text" placeholder="Promo Code" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} className="flex-1 rounded-md border border-gray-300 px-3 py-2 text-sm uppercase" /><button onClick={handleApplyCoupon} disabled={validatingCoupon || !couponCode} className="bg-black text-white px-4 py-2 rounded-md text-sm font-bold disabled:opacity-50">{validatingCoupon ? '...' : 'Apply'}</button></div> ) : ( <div className="flex justify-between items-center bg-green-100 p-2 rounded text-sm text-green-800"><span className="flex items-center gap-1"><Ticket size={14}/> Code <b>{appliedCoupon.code}</b> applied!</span><button onClick={() => setAppliedCoupon(null)} className="text-red-500 font-bold hover:underline">Remove</button></div> )} {couponError && <p className="text-xs text-red-600 font-bold">{couponError}</p>}
              </div>
            )}
            {step === 'success' && ( <div className="h-full flex flex-col items-center justify-center text-center animate-in zoom-in duration-300"><div className="bg-green-100 p-4 rounded-full mb-4"><CheckCircle size={48} className="text-green-600" /></div><h3 className="text-2xl font-bold text-gray-900">Order Placed!</h3><p className="text-gray-500 mt-2">Thank you, {formData.name}.</p><p className="text-gray-500">Order ID: #{orderId}</p><div className="bg-gray-50 p-4 rounded-lg mt-6 w-full text-left"><p className="text-sm text-gray-600">We will call you at <b>{formData.phone}</b> shortly to confirm delivery.</p></div><button onClick={onClose} className="mt-8 bg-black text-white px-6 py-2 rounded-full font-bold">Continue Shopping</button></div> )}
          </div>
          {step !== 'success' && cartItems.length > 0 && ( <div className="border-t border-gray-200 p-4 space-y-4 bg-white">{step === 'cart' ? ( <><div className="flex justify-between text-base font-medium text-gray-900"><p>Subtotal</p><p>৳ {subtotal}</p></div><button onClick={() => setStep('checkout')} className="w-full flex items-center justify-center rounded-md bg-black px-6 py-3 text-base font-medium text-white hover:bg-gray-800">Checkout <ArrowRight size={20} className="ml-2" /></button></> ) : ( <><button onClick={handlePlaceOrder} disabled={isSubmitting} className="w-full flex items-center justify-center rounded-md bg-green-600 px-6 py-3 text-base font-medium text-white hover:bg-green-700 disabled:bg-gray-400">{isSubmitting ? <><Loader2 className="animate-spin mr-2"/> Processing...</> : "Confirm Order"}</button><button onClick={() => setStep('cart')} disabled={isSubmitting} className="w-full text-center text-sm text-gray-600 hover:text-black">Back to Cart</button></> )}</div> )}
        </div>
      </div>
    </>
  );
...
const ProductModal = ({ product, isOpen, onClose, onConfirm }) => {
  ...
  return ( <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"><div className="absolute inset-0 bg-black bg-opacity-75 backdrop-blur-sm" onClick={onClose}></div><div className="relative bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col md:flex-row animate-in fade-in zoom-in duration-300"><button onClick={onClose} className="absolute top-4 right-4 z-10 p-2 bg-white rounded-full shadow-md hover:bg-gray-100"><X size={20} /></button><div className="w-full md:w-1/2 bg-gray-100 relative"><img src={product.image} alt={product.name} className={`w-full h-64 md:h-full object-cover ${isOutOfStock ? 'grayscale opacity-75' : ''}`} />{isOutOfStock && <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-10"><span className="bg-red-600 text-white font-bold px-4 py-2 text-lg transform -rotate-12 shadow-lg border-2 border-white">SOLD OUT</span></div>}</div><div className="w-full md:w-1/2 p-6 flex flex-col justify-between"><div><span className="text-sm text-gray-500 uppercase tracking-wider">{product.category}</span><h2 className="text-xl font-bold text-gray-900 mt-1">{product.name}</h2><p className="text-2xl font-bold text-gray-900 mt-2">৳ {product.price}</p><div className="mt-6"><h3 className="text-sm font-medium text-gray-900 mb-3">Select Size</h3><div className="grid grid-cols-3 gap-2">{['M', 'L', 'XL', 'XXL'].map((s) => ( <button key={s} onClick={() => !isOutOfStock && setSize(s)} disabled={isOutOfStock} className={`py-2 text-sm font-bold rounded border ${size === s ? 'bg-black text-white border-black' : 'bg-white text-gray-900 border-gray-200'} ${isOutOfStock ? 'opacity-50 cursor-not-allowed' : 'hover:border-black'}`}>{s}</button> ))}</div></div></div>{isOutOfStock ? ( <button disabled className="mt-8 w-full bg-gray-200 text-gray-500 font-bold py-3 rounded-lg cursor-not-allowed flex items-center justify-center gap-2"><AlertCircle size={20} /> Out of Stock</button> ) : ( <button onClick={handleAddToCart} className="mt-8 w-full bg-red-600 text-white font-bold py-3 rounded-lg hover:bg-red-700 transition-colors flex items-center justify-center space-x-2"><ShoppingBag size={20} /><span>Add to Cart</span></button> )}</div></div></div> );
};
    ...
    <div className="min-h-screen bg-white font-sans text-gray-900">
      ...
      <div className="relative bg-gray-900 text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-32 sm:px-6 lg:px-8 flex flex-col items-center text-center">
          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white drop-shadow-lg uppercase">
            {heroContent.hero_headline}
          </h1>
